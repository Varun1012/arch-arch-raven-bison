  ...
  class Game {
    ...
    updateSignal() {
      ...
      for (const s of this.storms) {
        ...
        if (s.kt >= 64 && km < SIG_KM[10]) sig = raiseSignal(sig, 10);
        else if (s.kt >= 48 && km < SIG_KM[9]) sig = raiseSignal(sig, this.cfg.skipNine ? 8 : 9);
        else if (s.kt >= 34 && km < SIG_KM[8]) sig = raiseSignal(sig, 8);
        else if (s.kt >= 22 && km < SIG_KM[3]) sig = raiseSignal(sig, 3);
        else if (km < SIG_KM[1]) sig = raiseSignal(sig, 1);
      }
